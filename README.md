...
**RPG de rotina universitária em pixel art • Protótipo v0.3.0**

Explore a república, o campus, a cidade e o Bar Aurora. Equilibre estudos, amizade, trabalho e entregas, administrando energia, dinheiro e risco.

Projeto editável para **Godot 4.5.1 Standard**, em GDScript. Sem plugins ou .NET. Protagonista jovem recem adulto e missão de pacote ficcional.

## Abrir

1. Neste repositório, use **Code → Download ZIP** e extraia os arquivos, ou clone com GitHub Desktop.
...
A aparência renderizada em janela e o uso no Windows ainda precisam de validação. As dimensões dos painéis foram verificadas pela engine, mas isso não equivale à inspeção visual. O repositório contém o projeto editável; ainda não há executável Windows ou APK distribuído.

## Checklist de continuidade

### Prioridade: estabilidade no Windows

- [ ] Importar o projeto e executar sem erros.
- [ ] Conferir cenário pixelado, personagem e HUD em janela.
- [ ] Fechar a introdução por teclado e mouse.
- [ ] Percorrer todas as áreas e alcançar todas as interações.
- [ ] Completar aula → aceitar pacote → entregar → dormir.
- [ ] Verificar diário, mochila e avaliação semanal.
- [ ] Reabrir o jogo e restaurar saves manual e automático.
- [ ] Registrar e corrigir qualquer falha bloqueante.

### Evolução planejada

- [ ] Converter arte procedural em sprites e tilesets editáveis.
- [ ] Separar jogador e áreas em cenas reutilizáveis.
- [ ] Criar animações completas em quatro direções.
- [ ] Ampliar narrativa, missões e rotina dos NPCs.
- [ ] Implementar patrulha móvel e segurança do campus.
- [ ] Adicionar áudio e configurações de acessibilidade.
- [ ] Exportar e testar uma versão Windows fora do editor.
- [ ] Adicionar controles de toque e testar em Android real.

Ainda não há áudio, NPCs andando, combate, missões variadas ou controles mobile. A bolsa é a consequência acadêmica inicial; não há reprovação ou expulsão.

## Desenvolvimento pelo GitHub

Use uma branch por tarefa, como `fix/dialogo` ou `feat/sprites-republica`. Antes de propor a integração à `main`, execute os testes e jogue o trecho alterado. Mudanças visuais devem acompanhar imagens ou vídeo.

Para relatar defeitos, abra uma Issue com:

- Versão do projeto e da Godot.
- Passos para reproduzir.
- Comportamento esperado e observado.
- Mensagem de erro completa e captura, quando aplicável.

Mantenha `project.godot`, scripts, cenas, recursos e arquivos `.uid` no repositório. A pasta `.godot/` é cache local e deve ficar fora do Git. Não envie saves pessoais, credenciais ou builds para a pasta do código.

O resultado de 47 testes acima corresponde à validação da entrega v0.3.0. Não representa uma execução automática do GitHub Actions; esse workflow ainda precisa ser configurado.
